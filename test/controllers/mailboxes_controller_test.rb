...
class MailboxesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create mailbox" do
    assert_difference('Mailbox.count') do
      post mailboxes_url, params: { mailbox: { data: @mailbox.data, name: @mailbox.name } }
    end

    assert_redirected_to mailbox_path(Mailbox.last)
  end
  ...
  test "should update mailbox" do
    patch mailbox_url(@mailbox), params: { mailbox: { data: @mailbox.data, name: @mailbox.name } }
    assert_redirected_to mailbox_path(@mailbox)
  end
